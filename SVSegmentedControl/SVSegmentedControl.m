...
@synthesize titlesArray, thumb, thumbRects, snapToIndex, trackingThumb, moved, activated, halfSize, dragOffset, segmentWidth, thumbHeight, imagesArray, selectedImagesArray;
...
- (id)initWithSectionImages:(NSArray *)images {
    ...
    if (self) {
        ...
        self.selectedImagesArray = [NSMutableArray arrayWithArray:images];
        self.thumbRects = [NSMutableArray arrayWithCapacity:[images count]];
        self.accessibilityElements = [NSMutableArray arrayWithCapacity:self.imagesArray.count];
...
- (void)initTitles {
    ...
    self.segmentWidth = (CGFloat) (ceil(self.segmentWidth/2.0)*2); // make it an even number so we can position with center
...
- (void)initImages {
    ...
    for(UIImage *image in self.imagesArray) {
        ...
        self.height = MAX(imageHeight, self.height);
    }
    ...
    self.thumb.frame = [[self.thumbRects objectAtIndex:0] CGRectValue];
    self.thumb.layer.shadowPath = [UIBezierPath bezierPathWithRoundedRect:self.thumb.bounds cornerRadius:2].CGPath;
    self.thumb.imageView.image = [self.selectedImagesArray objectAtIndex:0];
...
- (void)drawRect:(CGRect)rect {
    ...
    if (_isImages)
        ...
        [self drawTitles:rect];
}

- (void)drawTitles:(CGRect)rect {
    CGFloat posY = (CGFloat) (ceil((CGRectGetHeight(rect)-self.font.pointSize+self.font.descender)/2)+self.titleEdgeInsets.top-self.titleEdgeInsets.bottom);
...
- (void)drawImages {
    ...
    for (UIImage *image in self.imagesArray) {
        ...
        CGRect imageRect = CGRectMake((self.segmentWidth * i) + (self.segmentWidth - image.size.width) / 2, (self.segmentWidth - image.size.height) / 2, image.size.width, image.size.height);
...
- (id)accessibilityElementAtIndex:(NSInteger)index {
    ...
    CGFloat posY = (CGFloat) (ceil((CGRectGetHeight(self.bounds)-self.font.pointSize+self.font.descender)/2)+self.titleEdgeInsets.top-self.titleEdgeInsets.bottom);
...
- (void)endTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
    ...
    if(!self.moved && self.trackingThumb && ([self.titlesArray count] == 2 || [self.imagesArray count] == 2))
        [self toggle];
...
- (void)snap:(BOOL)animated {
    ...
    if(self.snapToIndex != -1)
        ...
        index = (int) floor(self.thumb.center.x/self.segmentWidth);

    if (_isImages)
        self.thumb.imageView.image = [self.selectedImagesArray objectAtIndex:index];
    else
...
- (void)updateTitles {
    int hoverIndex = (int) floor(self.thumb.center.x/self.segmentWidth);
    ...
    if (secondTitleOnLeft && hoverIndex > 0) {
        ...
        self.thumb.secondLabel.text = [self.titlesArray objectAtIndex:(NSUInteger) (hoverIndex - 1)];
    ...
    self.thumb.label.text = [self.titlesArray objectAtIndex:(NSUInteger) hoverIndex];
}
...
- (void)updateImages {
    ...
    if (secondTitleOnLeft && hoverIndex > 0) {
        self.thumb.imageView.alpha = 0.5 + ((self.thumb.center.x / self.segmentWidth) - hoverIndex);
        self.thumb.secondImageView.image = [self.selectedImagesArray objectAtIndex:(NSUInteger) (hoverIndex - 1)];
        self.thumb.secondImageView.alpha = 0.5 - ((self.thumb.center.x / self.segmentWidth) - hoverIndex);
    }

    else if (hoverIndex + 1 < self.imagesArray.count) {
        self.thumb.imageView.alpha = 0.5 + (1 - ((self.thumb.center.x / self.segmentWidth) - hoverIndex));
        self.thumb.secondImageView.image = [self.selectedImagesArray objectAtIndex:(NSUInteger) (hoverIndex + 1)];
        self.thumb.secondImageView.alpha = ((self.thumb.center.x / self.segmentWidth) - hoverIndex) - 0.5;
    }

    else {
        self.thumb.secondImageView.image = nil;
        self.thumb.imageView.alpha = 1.0;
    }

    self.thumb.imageView.image = [self.selectedImagesArray objectAtIndex:(NSUInteger) hoverIndex];
}

- (void)activate {

    self.moved = NO;
    self.trackingThumb = self.moved;

    if (_isImages)
        self.thumb.imageView.image = [self.selectedImagesArray objectAtIndex:self.selectedIndex];
    else
